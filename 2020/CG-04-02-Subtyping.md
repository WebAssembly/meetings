...
AR: Want to continue.. Let’s not make this structural/nomical discussion
...
PP: Can you elaborate on what the C/C++ was?

AR: Basically, you need to be able to pass values back and forth between C code and Engine, you need a union. What to do about references, you have one field of references. If you have an open-ended set you have a growing number of arguments in the union. It also shows up in how we deal with imports/exports, for example what we call functions, tables, globals. You can use the same reference values there too. I spent some time coming up with an alternative there, but I have failed to come up with anything nice.

PP: This is .. <muffled> A good enough level of abstraction, makes sense for alignment etc. 

AR: We will eventually need this, no way around that - roadmap for 3 years. 
...
AR: A natural way to represent them as functions right?

RT: Usually capabilities are a table of functions, not just one, right?

AR: Broader point is that we shouldn’t make premature decisions of what we might do
...
AR: For anyref subtyping removal to make sense you also have to remove nullref - the intent is to partition hierarchy, so if you remove it it doesn’t make sense
...
AR: The other part of the question is whether to remove subtyping altogether. But it leaves the proposal in an incoherent state. It means we have subtyping in the proposal but no subtyping rules. It doesn't make sense to have subtyping you can't observe, so you'd want to remove it altogether. You'd also want to rename anyref to something else. These are at least three further steps to remove clean it up. This is just the effect on this proposal itself -- we still have other effects.

Bulk memory has a smaller change, exceptions proposal - rules about exnref that would have toc hange. Func ref, the work there is completely invalidated, would have to move that over and redo. Type imports, not much work there but it is a deeply impacted proposal. Also mentioned C++ API etc. At least 3 and a half are already implemented, they all would be impacted - all the implementations have to change. More churn - so we should seriously consider if we want to impose the change. Not a question of a couple of days, at least a couple of weeks for every implementation. This is the roadmap we had agreed on, by abandoning some aspect of it - what do we do with the rest of the roadmap - do we want to reconsider the entire roadmap? 
...
AR: We probably need to at some point. But if we decide that this is not an interesting choice, we can add a variant of function type that is subtypable or not. We can play around with options or not.
...
AR: That depends on what we figure out... if we figure out that we can implement this efficiently, then we will make it work. if not ...
...
RT: You are making us commit to this choice soon.. <AR: Disagrees>
...
AR: Once we get to the point, we have to come up with a coherent solution. 

RT: With every system.. If you make decisions one step at a time.. It’s regretted, this will come up soon

AR: This will come up in the GC proposal
