...
AR: This has nothing to do with type variables, if you care about what are all the types of T*.. but this is independent of that. If you want to represent the principal split type, I haven’t thought about that yet, but you may need to enumerate all the types

PP: wrt typed references: we’re not talking only about really complex types like structs?

AR: The reference types, they could be anything right? In the func ref proposal they can denote funcs, in the GC proposals they could be structs, arrays, etc, in the typed continuations, they could denote continuations etc., so more general
...
AR: We should document it, that is the outcome we had, the desirable properties are the ones we have, we could drop that, but at least we have to think carefully of why you would need it
...
AR: for each one in the desirable category, we know what would break them. For principal types, what would break it is annotations. For GLBs you would need to go full intersection types. Decomposition you would weaken if you weaken the type system of the unreachable code.
...
AR: I agree that you’re bound to run into a wall there if you don’t have the GLB property

TL: Is there any way to weaken the current GLB property without LUBs? Or do they imply each other?

AR: currently they don’t imply each other, since we have explicitly declared subtypes, when you have contravariance you have to declare it. The subtype relation only depends on wha you’ve declared. But you would be able to declare it, and certain subtypes you wouldn't be able to declare anymore in the presence of contravariance if you don’t have GLBs 
...
AR: Well no, if the whole sequence has an input/output type and that consumes something that if the right consumes something that the left doesn’t touch, it just ignores it. There are these frame like subtyping rules <AR: to add rule here> it’;s basically a supertype of the more precise one. You lose the information for the more precise types otherwise

TL: how does this interact/what is the status of the proposal to simplify dead code validation?

AR: That breaks decomposition, as for the status Conrad? 
...
MD:: I’m not sure this applies here but one case is where the sequence serves as a sink for the value and needs to accept any value
...
AR: since this is only relevant for types that are not bottom, it’s independent… why would you do that unreachable code? So it’s not really related to unreachable code. As long as we keep the property that any type of a value can be a parameter of the function it should be ok. Do you have an example where this breaks?

MD: IT triggers internal compiler errors at times

AR: Would like to see an example
...
AR: The negative value checks the unreachable code, not sure about the exact tests are there, but there are tests there that test them, and I would be surprised if engines pass it without implementing
...
AR: The status quo is that there is no special rule in the types system, the implementations might need a flag but the type system doesn’t care. I would say that the new version makes it more complicated
...
AR: My concern is that you have far more places where you have to make a case distinction, and easy to make mistakes, whereas with the current status there are places where we have to do something but probably fewer because you don’t have two mdes

CW: My impression is that a lot of implementations do it now by having a conditional flag based on whether the current code is reachable. My hope was to make that unnecessary. They may still have som ethin glike that but, hopefully it would still be simpler.
