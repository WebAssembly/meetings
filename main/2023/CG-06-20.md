...
[PP showing example]
PP: This is an example from the ??? presentation, so you would know the vector, a SIMD register. So you would load, the amount of bytes required to fill it, then perform the operations and store to a different SIMD register. And store the amount of bytes. Basically if you have different lengths, you’d check that. If you have different lengths, then you are suspended and have a different architecture?
