...
EP: For our purposes here, “TR” means w3.org TR; it means a doc that conforms to publication rules.
The previous doc (the rec) is big and unwieldy. I decided to try to write a sphinx to TR tool to convert it automatically: https://github.com/w3c/sphinx-to-tr example output is at http://uu3.org/checkouts/w3c/sphinx-to-tr/out/
...
EP: technically they are orthogonal. Auto-publication just lets you keep it up to date when you iterate on a working draft. Evergreen is that you can stay in CR, and not have to go to REC every couple years, etc. 

LW: we don’t merge stuff too much, it’s mostly like we just did, with a “really done” chunk of work.

EP: yeah when you go to CR you stabilize for 3 months, to update accessibility, internationalization, keep a summary of what’s changed, etc. With evergreen, the patent commitment and review process you did to get to CR, you want to make sure you don’t invalidate the previous patent/accessibility/i18n reviews etc by starting everything over or whatever.
...
EP: sort of. It’s what the current version does with mathjax. In the future it might be possible to do it AOT. or we could have both a mathML and mathjax version.
...
EP: The decision means you just let me know when it’s merged and I’ll try to fix up the rest of the script, and get the webmaster to accept the pubklication request.
There will be a 2 week wait for the director to [??]
